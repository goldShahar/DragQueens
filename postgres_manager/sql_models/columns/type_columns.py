...
class TypeNameColumn(TableColumn):
    def read_value(cls, value_to_filter: list[str]):
        return lambda: Type.type_name.in_(value_to_filter)
    

class ImportanceColumn(TableColumn):
    def read_value(cls, value_to_filter: list[str]):
        return lambda: get_correct_cond(int, Type.importance, value_to_filter)
    

class MinTimeColumn(TableColumn):
    def read_value(cls, value_to_filter: list[str]):
        return lambda: get_correct_cond(int, Type.Min_time, value_to_filter)
    

class MinAreaColumn(TableColumn):
    def read_value(cls, value_to_filter: list[str]):
        return lambda: get_correct_cond(float, Type.Min_area, value_to_filter)
