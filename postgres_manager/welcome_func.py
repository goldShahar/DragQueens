...
class ActionToTableSingleton:
    ...
    actions_dict: dict[str, callable] = {
        "read + type": read_type,
        "insert + type": create_type,
        "delete + type": delete_type,
        "read + hazard": read_hazard,
        "insert + hazard": create_hazard,
        "delete + hazard": delete_hazard,
    }
...
def welcome_func(action: str, table: str, values: dict[str, Any]):
    ...
    print("values", values)
    try:
        return ActionToTableSingleton().actions_dict.get(
            f"{action.lower()} + {table.lower()}"
        )(values)
