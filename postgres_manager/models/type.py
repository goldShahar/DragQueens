...
def delete_type(type_name: str):
    if not read_type(type_name=type_name, selected_columns=[True, False, False, False]):
...
def read_type(type_name: str | None, Importance: int | None, Min_time: datetime | None, Min_area: float | None, selected_columns: list[bool]):
    return read_type_from_pg([type_name, Importance, Min_time, Min_area], selected_columns)
